...
class MyRSI(MyIndicator):
    ...
    def calculate_indicator(self) -> dict[str, DataFrame]:
        ...
        # 遍历不同的时间周期，计算相对强弱指标
        for period in self.product_df_list.keys():
            # 获取daily和weekly的ohlc数据
            df_ohlc = self.product_df_list[period]
            # 创建一个新的DataFrame，用于存储RSI_short_term和RSI_long_term的数据
            df_rsi = DataFrame(
                index=self.product_df_list[period].index,
                columns=[
                    "difference",
            ...
            df_rsi[f"avg_gain_{short_term}"] = self._wilder_smoothing_method(
                positive_difference, short_term
            ).round(4)
            df_rsi[f"avg_loss_{short_term}"] = self._wilder_smoothing_method(
                negative_difference, short_term
            ).round(4)
            df_rsi[f"avg_gain_{long_term}"] = self._wilder_smoothing_method(
                positive_difference, long_term
            ).round(4)
            df_rsi[f"avg_loss_{long_term}"] = self._wilder_smoothing_method(
            ...
            # 将df_rsi保存为csv文件
            with open(
                file=f"{self.data_path}\\{self.product_code}{period[0].upper()}_{self.today_date.strftime('%m%d')}_RSI.csv",
                mode="w",
                encoding="utf-8",
            ) as f:
                df_rsi.to_csv(f)
...
if __name__ == "__main__":
    MyRSI(None, dt.date.today(), "002230", ProductType.Stock).calculate_indicator()
